...
*/ 
__ALIGN_BEGIN static const uint8_t HID_AGENT_ReportDesc[] __ALIGN_END =
{
...
const USBD_Class_cb_TypeDef  USBD_AGENT_cb = 
{
  USBD_HID_Init,
  USBD_HID_DeInit,
  USBD_HID_Setup,
...
__ALIGN_BEGIN static const uint8_t USBD_HID_CfgDesc[USB_AGENT_CONFIG_DESC_SIZ] __ALIGN_END =
{
  0x09, /* bLength: Configuration Descriptor size */
  USB_CONFIGURATION_DESCRIPTOR_TYPE, /* bDescriptorType: Configuration */
  USB_AGENT_CONFIG_DESC_SIZ,
  ...
  sizeof(HID_AGENT_ReportDesc),/*wItemLength: Total length of Report descriptor*/
  0x00,

  0x07,          /*bLength: Endpoint Descriptor size*/
  USB_ENDPOINT_DESCRIPTOR_TYPE, /*bDescriptorType:*/

  HID_AGENT_IN_EP,     /*bEndpointAddress: Endpoint Address (IN)*/
  0x03,          /*bmAttributes: Interrupt endpoint*/
  HID_AGENT_IN_PACKET, /*wMaxPacketSize: 4 Byte max */
  0x00,
  ...
  HID_AGENT_OUT_EP,     /*bEndpointAddress: Endpoint Address (IN)*/
  0x03,          /*bmAttributes: Interrupt endpoint*/
  HID_AGENT_OUT_PACKET, /*wMaxPacketSize: 4 Byte max */
  0x00,
  0x01,         /*bInterval: Polling Interval (1 ms)*/
  /* 34 */
} ;
                                 ...
                                 uint8_t cfgidx)
{
  ...
  DCD_EP_Close (pdev , HID_AGENT_OUT_EP);
                                ...
                                USB_SETUP_REQ *req)
{
  ...
  case USB_REQ_TYPE_STANDARD:
    ...
    case USB_REQ_GET_DESCRIPTOR: 
      ...
      {
        len = MIN(sizeof(HID_AGENT_ReportDesc) , req->wLength);
        pbuf = HID_AGENT_ReportDesc; // wiiccReportDescriptor; //
      }
...
uint8_t USBD_AGENT_SendReport(USB_OTG_CORE_HANDLE  *pdev, uint8_t * report, uint16_t len)
{
  if (pdev->dev.device_status == USB_OTG_CONFIGURED) {
    if (ReportSent) {
      if (report) {
        ReportSent = 0;
        DCD_EP_Tx (pdev, HID_AGENT_IN_EP, report, len);
      }
      return USBD_OK;
    }
  }
                              ...
                              uint8_t epnum)
{
  ...
  DCD_EP_Flush(pdev, HID_AGENT_IN_EP);
  return USBD_OK;
}
