...
int checkIncDec(event_t event, int val, int i_min, int i_max, unsigned int i_flags, IsValueAvailable isValueAvailable, const CheckIncDecStops &stops)
{
  ...
  else if (s_editMode>0 && ((event==EVT_ROTARY_LEFT && !rotary_reverse) || (event==EVT_ROTARY_RIGHT && rotary_reverse))) {
    newval -= (min<int>(rotencSpeed, val-i_min) * param_step_value);
