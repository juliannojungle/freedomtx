...
void menuRadioSetup(event_t event)
{
  ...
  MENU(STR_MENURADIOSETUP, menuTabGeneral, MENU_RADIO_SETUP, ITEM_SETUP_MAX, { 
    2, 2, 0, 1, 
    ...
    CASE_PXX(0) 0, 0, 
    IF_FAI_CHOICE(0) 0, 0, 0, 
    LABEL(TX_MODE), 0, 1/*to force edit mode*/ });
  ...
  for (uint8_t i=0; i<NUM_BODY_LINES; i++) {
    ...
    switch (k) {
      ...
      case ITEM_SETUP_BATTERY_RANGE:
        ...
        putsVolts(lcdLastRightPos+2*FW, y, 120+g_eeGeneral.vBatMax, (menuHorizontalPosition>0 ? attr : 0)|LEFT|NO_UNIT);
        if (attr && menuHorizontalPosition < 0) lcdDrawFilledRect(RADIO_SETUP_2ND_COLUMN, y, LCD_W-RADIO_SETUP_2ND_COLUMN-MENUS_SCROLLBAR_WIDTH, 8);
        if (attr && s_editMode>0) {
  ...
  #if defined(PCBTANGO)
        if (attr) CHECK_INCDEC_GENVAR(event, g_eeGeneral.backlightMode, e_backlight_mode_keys, e_backlight_mode_on);
  #endif
       break;
